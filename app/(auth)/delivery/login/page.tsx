  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        toast.success("Delivery partner session active.");
        ...
        if (role !== "DELIVERY") {
          ...
          toast.error("Unauthorized. Delivery Partner privileges required.");
          return;
        }
    ...
    <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-slate-50 to-[#CAF0F8] dark:from-slate-950 dark:to-slate-950 p-4">
      <div className="max-w-md w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-sky-950 rounded-2xl shadow-xl p-6 md:p-8 space-y-6">
        ...
        <div className="flex flex-col items-center text-center space-y-2">
          <Link href="/" className="w-12 h-12 rounded-2xl bg-[#00B4D8] flex items-center justify-center text-white shadow-md">
            <Truck size={24} />
          </Link>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Sign in to access assigned deliveries and scanner utility.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <input
                id="email"
                type="email"
                placeholder="driver@aquahome.com"
                {...register("email")}
          ...
          <button
            type="submit"
            className="w-full py-3 bg-[#00B4D8] hover:bg-[#0096C7] text-white rounded-xl text-sm font-semibold transition shadow-md flex items-center justify-center"
            disabled={loading}
          >
            {loading ? (
